...
`enctype` is an attribute of the **`<form>` element**. It tells the browser **how to encode/package the form data before sending it to the server**.

Think of it as:

> **`enctype` = "In what format should I send this form's data?"**
...
The data is sent approximately like:

```text
username=Sumit&email=sumit%40gmail.com
```

You normally **don't need to write this**, because it's the default for normal forms.

---

### 2. `multipart/form-data` — for file uploads

This is the important one when uploading files.
...
Here the browser sends the form as **multiple parts**, allowing the actual file data to be included.
...
If you're sending the file to a server, you normally use:

```html
enctype="multipart/form-data"
```

Otherwise, the file isn't transmitted in the appropriate multipart format.
...
The data is sent in a simple text format, roughly:
...
This is **not commonly used for production forms**.

---

### The three standard values

| `enctype`                           | Main purpose                      |
| ----------------------------------- | --------------------------------- |
| `application/x-www-form-urlencoded` | Normal form data (**default**)    |
| `multipart/form-data`               | **File uploads**                  |
| `text/plain`                        | Plain-text form data; rarely used |

### The easiest rule for students
...
Form contains file upload
      ↓
multipart/form-data
```

For example, if you're uploading a **PDF resume**:
...
That's the most important real-world use of `enctype`.
...
**`form` attribute on an `<input>`**, it is used to connect an input to a specific `<form>` element **even when the input is outside that form**.
...
<input type="email" name="email" form="loginForm">
```

The email input is **outside the `<form>`**, but:
...
So when you click **Login**, the browser submits:
...
The input is physically inside the form.

But sometimes your HTML layout requires the input to be somewhere else:
...
Email input is somewhere else on the page
```

You can connect it using:

```html id="iyqkfp"
<input name="email" form="loginForm">
```

### Important: `form` needs the form's `id`
...
> **`form` attribute on an input tells the browser which `<form>` the input belongs to.**

It is especially useful when the input **cannot physically be placed inside the form in your HTML structure**.



`formaction` is used on a **submit button/input** to specify a **different URL where the form data should be sent**.

Normally, the destination is defined by the form:

```html
<form action="/submit">
```

But `formaction` lets a particular submit button **override** that `action`.
...
Both are part of the **same form**, but the button decides where the data goes.

### Why is this useful?

Imagine an admin form:
...
<form action="/save-user" method="post">
  ...
  <button type="submit" formaction="/preview-user">
    Preview
  </button>
...
The user **doesn't submit twice**. They choose the action they want.

### `action` vs `formaction`
...
<button formaction="/preview">
       ↓
   Override destination for THIS button
```
...
| ------------ | ----------------------------- | ------------------------------- |
| `action`     | `<form>`                      | Default URL for form submission |
| `formaction` | Submit `<button>` / `<input>` | Overrides the form's `action`   |

### Important

`formaction` works with submit controls:
...
> **`formaction` allows a particular submit button to send the form data to a different URL than the form's default `action`.**
...
`formmethod` tells the browser **which HTTP method to use when that particular button submits the form**.
...
But a submit button can override it.
...
If the user clicks **Create User**:
...
<button formmethod="get">
       ↓
Override method for this button
```

### When is it useful?

When the same form has different actions.
...
`formnovalidate` tells the browser:

> **"Don't perform the browser's built-in form validation when this particular button is clicked."**
...
If the email is empty and you click:

### Submit

The browser stops submission:
...
The browser doesn't perform its normal HTML validation because:
...
This makes `formnovalidate` very useful for things like **Save Draft**, where you don't require the user to complete every field.

---

## Important distinction

`formnovalidate` **doesn't mean the server won't validate the data**.

It only disables **browser/client-side HTML validation**.
...
The browser normally checks that the field is filled and looks like an email.
...
that browser check is skipped.

But your backend can—and generally should—still validate the submitted data.

---

### Easy way to teach all these `form*` attributes

You can show students this pattern:
...
`formtarget` tells the browser **where to open/display the response when a particular submit button submits the form**.

It works similarly to the `target` attribute of `<form>`, but lets **one particular submit button override it**.
...
* **Submit** → opens the response in the current tab.
* **Submit & Open New Tab** → opens the response in a **new tab** because of `formtarget="_blank"`.

### A practical example: Preview
...
```text
Form → /preview → current tab
```

When the user clicks **Preview**:

```text
Form → /preview → new tab
```

So the user can keep their form open while seeing the preview in another tab.

### Common `formtarget` values
...
```
 ...
 Parent browsing context, useful when working with frames/iframes.
...
> **`formtarget` specifies where the response from a form submission should be displayed for a particular submit button.**

And just like the other `form*` attributes:
